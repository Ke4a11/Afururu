package ke4a11.ecc.ac.jp.afururu.English;


public class Eng_Text {

    /*
    *EngHotel List01
     */

    //お気に入り左
    public static final String[] TITLES_favorite =
            {

            };
    //お気に入り右
    public static final String[] DIALOG_favorite =
            {

            };


    public static final String[] TITLES_hotel =
            {
                    //hotel_checkin(黒色)
                    "チェックインをお願いします。名前は_____です。",
                    "これが予約確認書です。（予約してある場合）",
                    "予約してません。空いている部屋はありますか？（予約していない場合）",
                    "シングル１部屋で、２泊です。",
                    "バスルーム/シャワーつきの部屋ですか？",
                    "ご予約頂いている_______様ですね。",//会話
                    "空いている部屋はございます。どうぞ、お泊まり下さい。",//会話
                    "あいにく、空いている部屋はございません。",//会話
                    "こちらのフォームにご記入下さい。",//会話
                    "お支払はクレジットカードですか？",//会話
                    "お部屋の番号は＿＿＿になります。こちらが鍵です。",//会話
                    "hotel_checkin",
                    //hotel_stay(白色)
                    "（今から外出します。）鍵を預かって下さい。",
                    "（外出から帰り、）鍵を下さい。部屋の番号は________です。",
                    "タクシーを呼んでもらえますか？",
                    "この近くにおすすめのレストランありませんか？",
                    "明日朝、モーニングコールを______時にお願いします。部屋番号___の＿＿＿＿です。",//16
                    "hotel_stay",
                    //hotel_claim(赤色)
                    "部屋に鍵を置いたままドアを閉めてしまいました。",
                    "部屋に困った事があるのですが。",
                    "エアコンが効いていません。",
                    "冷蔵庫の調子が悪いようです。",
                    "トイレがよく流れません。",
                    "バスタブの水が流れません。",
                    "直していただけますか？",
                    "隣の部屋の人がとても騒がしいのですが、別の部屋に替え\n" +
                            "ていただけますか？\n",
                    "部屋がまだ掃除されていません。",
                    "ベッドメイクができていません。すぐにベッドメイクしてください。",
                    "部屋にタオルがありません。持ってきてください。",
                    "ドアの鍵がかかりません。",
                    "hotel_claim",
                    //hotel_checkout(青色)
                    "チェックアウトお願いします。",//会話
                    "有難うございます。少々お待ち下さい。",//会話
                    "料金は_________ドルとなります。\n" +
                            "明細書はこちらです。\n",
                    "料金に計算違いがある様なんですが。",
                    "これは何の料金ですか？（明細を見せながら）",
                    "電話は使いませんでしたよ。",
                    "映画は見ていないです。",
                    "ミニバーのコーラをお飲みになりましたので、その料金です。",
                    "それでは、こちらにサインをお願いします。",
                    "hotel_checkout"


            };

    /*
    *EngHotel List02
     */


    public static final String[] DIALOG_hotel =
            {
                    "チェックインをお願いします。名前は_____です。\n" +
                            "I'd like to check in. My name is _________.",
                    "This is a confirmation slip.",
                    "I have no reservation. Do you have a room available?",
                    "One single room for 2 nights.",
                    "Is this room with a bath / shower?",
                    "Mr (Ms)____________. Yes, we have your reservation.",
                    "We have a room available. We can provide a room for you.",
                    "Unfortunately, we have no rooms available.",
                    "Please fill out this form.",
                    "Will you make payment by credit card?",
                    "Your room number is ___. This is the room key.",
                    //ホテル滞在中
                    "(I will go out now.) Please keep my key, thanks.",
                    "(Coming back to the hotel) Can I have my key? My room number is __________.",
                    "Would you call me a taxi please?",
                    "Do you have any suggestions for good restaurants in the area?",
                    "Can I ask you for a wake-up call at ____ am tomorrow morning? My name is _____, room number is ___.",//16
                    //クレーム対応
                    "I left the key inside the room.",
                    "I have a problem in my room.",
                    "The air conditioner doesn't work.",
                    "There's something wrong with the air conditioner.",
                    "The toilet doesn't flush well.",
                    "The water in the bathtub doesn't drain.",
                    "Could you fix it?",
                    "The people next door are very noisy.\n" +
                            "Could you get me a different room?\n",
                    "What is this charge for? (showing the statement?)",
                    "I didn't make any phone calls.",
                    "I didn't watch any movies.",
                    "You had a coke from the mini bar. This is fee.",

            };

    /*
    *EngAirport List01
     */
    public static final String[] TITLES_airport =
            {
                    "パスポートを見せてください。",
                    "どうぞ",
                    "訪れた目的はなんですか？",//3～7まで会話
                    "観光です。",
                    "休暇です。",
                    "仕事です",
                    "留学です",//3～7まで会話
                    "この国にどのぐらい滞在しますか？",
                    "6日間です。／3週間ぐらいです。",
                    "乗り継ぎをするだけです。",
                    "トランジットだけです。",
                    "どこに滞在する予定ですか？",
                    "オスカーホテルです。",
                    "これから探す予定です。",
                    "帰りのチケットはお持ちですか？",//ここから
                    "はい、これです。",
                    "いいえ、帰りにニューヨークで買うつもりです。",//ここまで会話
                    "職業はなんですか？",
                    "会社員です。",
                    "英語がよくわかりません。",
                    "日本語が話せる人はいますか？",
                    "お飲み物はいかがですか？",//飛行機内での会話
                    "どんな飲み物がありますか？",
                    "水をお願いします",
                    "コーヒー（お茶）をお願いします。",
                    "赤ワインをお願いします。",


            };
    public static final String[] DIALOG_airport =
            {

                    "Show me your passport,please.\nMay I see your passport?",
                    "Here you are.",
                    "What's the purpose of your visit?",//ここから
                    "Pleasure.／Sightseeing.",
                    "On vacation.",
                    "Business.",
                    "To study.",//ここまで会話
                    "How long will you stay in this country?\nHow long are you going to stay in this country?",
                    "For six days.／About three weeks.",
                    "I'm just passing through.",
                    "Just to transit.",
                    "Where are you going to stay?",
                    "At the Oscar Hotel.",
                    "I'll look for the hotel from now on.",
                    "Do you have a return ticket?",//ここから
                    "Yes,here you are.",
                    "No,I'll get it in New York.",//ここまで会話
                    "What is your occupation?",
                    "I'm an office worker.",
                    "I'm not good at English.\nI'm poor at English.",
                    "Does anybody here speak Japanese?\nAre there those who can speak Japanese",
                    "Would you like something to drink?\nWhat would you like to drink?",//飛行機内での会話
                    "What kind of drink do you have?",
                    "Water, please.",
                    "Coffee (Tea), please",
                    "Red wine, please.",


            };

    public static final String[] TITLES_life =
            {
                    "今日は予定があります",//今日の予定や帰宅時間を話すには？
                    "今日は遊園地に行きます（行く予定です）",
                    "午後６時くらいに家に戻ります",
                    "午後７時までに家に戻ります",
                    "まっすぐ進んでください",//道案内
                    "～で左・右に曲がってください",
                    "二つ目の信号を右に曲がってください",
                    "～が見えます",
                    "～の隣です",
                    "銀行の隣です",
                    "~の向かいです",
                    "銀行の向かいです",
                    "AとBとの間にあります",
                    "学校と銀行の間にあります",
                    "～の角にあります",
                    "角に病院があります",
                    "道の終わりにあります。",
                    "～に突き当たります",




            };
    public static final String[] DIALOG_life =
            {
                    "I have plans today.",//今日の予定や帰宅時間を話すには？
                    "I'm gonna go to an amusement park today",
                    "I will come home about 6pm.",
                    "I will come home by 7pm.",
                    "Go straight.",
                    "Turn left/right at ～.",
                    "Turn right at the second traffic lights.",
                    "You will see.",
                    "It's next to ～",
                    "It's ext to the bank.",
                    "It's opposite/across from ～",
                    "It's opposite the bank.",
                    "It's between A and B.",
                    "It's between the school and the bank.",
                    "~ on the corner",
                    "There is a hospital on the corner",
                    "It's at the end of the street/road.",
                    "You will come to ～",

            };
    public static final String[] TITLES_emergency =
            {
                    "緊急です",
                    "警察を呼んでください",
                    "医者を呼んでください",
                    "救急車を呼んでください",
                    "日本語の話せる人はいますか",
                    "遺失物係はどこですか",
                    "日本大使館はどこですか",
                    "助けて!",//助けを呼ぶときの言い方
                    "どうしたんですか？",//助けを呼ぶときの言い方
                    "泥棒！",//トラブルを話すときの英会話
                    "強盗です！",
                    "昨日、バッグが盗まれました",
                    "パスポートをなくしました",
                    "カードを盗まれました。カードを無効にしてください",
                    "具合が悪いんです",//症状を英語で説明するための英会話
                    "おなかの調子が悪いんです",
                    "熱が高いんです",
                    "食中毒なんです",
                    "彼は車にひかれたんです",
                    "車が自転車にぶつかりました",
                    "彼はひどいけがをしています",
                    "彼女は意識がありません",
                    "どうも左足を骨折したようなんです",
                    "肘の骨が折れました",
                    "彼女は右手にひどくやけどしました",

            };
    public static final String[] DIALOG_emergency =
            {
                    "It's an emergency.",
                    "Call the police.",
                    "Please call a doctor.",
                    "Please call an ambulance.",
                    "Does anyone here speak Japanese?",
                    "Where is the Lost and Found?",
                    "Where is the Japanese Embassy?",
                    "Help me!",//助けを呼ぶときの言い方
                    "What happened?",//助けを呼ぶときの言い方
                    "Thief!",
                    "Robber!",
                    "I had my bag stolen yesterday.",
                    "I have lost my passport.",
                    "My credit card has been stolen.\nMy credit card has been stolen.",
                    "I feel sick.",//症状を英語で説明するための英会話
                    "I have an upset stomach.",
                    "I have a high temperature.",
                    "I have food poisoning.",
                    "He was hit by a car.\nHe was run over by a car.",
                    "A car hit my bicycle.",
                    "He has a terrible injury.",
                    "He is unconscious.",
                    "I think I might have fractured my left leg.",
                    "I broke my elbow.",
                    "She burned her right hand badly.",

            };
    public static final String[] TITLES_electrictrain =
            {
                    "～までいきたいのですが。",
                    "～までの料金はいくらですか",
                    "～までの片道キップをください",
                    "○駅までの往復キップをください.",
                    "発車時刻は何時ですか？",
                    "時刻表はありますか？",
                    "乗換えが必要ですか？",
                    "始発（終電）は何時ですか？",
                    "～行きは何番線ですか？",
                    "3番線ホームはどこですか？",
                    "この電車は○○に停まりますか",
                    "ここに座ってもいいですか？",
                    "ここは私の席だと思うのですが。",
                    "次の駅はなんですか？",


            };

    public static final String[] DIALOG_electrictrain =
            {
                    "I’d like to go to ○○.",
                    "What’s the fare for ○○?",
                    "One way ticket for ○○, please.",
                    "Round trip ticket for ○○, please.",
                    "What time does the train leave?",
                    "Do you have a timetable?",
                    "Do I need to change trains?",
                    "What time is the first(last) train?",
                    "Which line is for ○○?",
                    "Where is train line number three?",
                    "Does this train stop at ○○?",
                    "May I sit here?",
                    "I think,this is my seat.",
                    "What is the next station?",


            };
    public static final String[] TITLES_res =
            {
                    "ホテルセンテンス",


            };

    public static final String[] DIALOG_res =
            {
                    "I’d like to go to ○○.",

            };

}
